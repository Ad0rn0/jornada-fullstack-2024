...
namespace Fina.Api.Endpoints.Transactions
{
	public class CreateTransactionEndpoint : IEndPoint
	{
		...
		{
			app.MapPost("/", HandleAsync).WithName("Transaction: Create").WithSummary("Cria uma nova transação").WithDescription("Cria uma nova transação").WithOrder(1).Produces<Response<Transaction?>>();
		}
