...
namespace Fina.Api.Handlers
{
	public class CategoryHandler(AppDbContext context) : ICategoryHandler
	{
		public async Task<Response<Category?>> CreateAsync(CreateCategoryRequest request)
		{
			try
			{
				/* Versão alternativa da instanciação de uma classe.
			...
			{
				return new Response<Category?>(null, 500, ex.Message/* "Não foi possível criar a categoria" */);
			}
		}
		...
		public async Task<PagedResponse<List<Category>?>> GetAllAsync(GetAllCategoriesRequest request)
		{
			try
			{
				...
				var categories = await query.Skip(request.PageCount).Take(request.PageSize).ToListAsync();
