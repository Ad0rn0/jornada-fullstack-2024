...
namespace Fina.Web.Handlers
{
	public class CategoryHandler(IHttpClientFactory httpClientFactory) : ICategoryHandler
	{
		...
		{
			return await _client.GetFromJsonAsync<PagedResponse<List<Category>?>>("v1/categories") ?? new PagedResponse<List<Category>?>(null, 400, "Falha ao obter categorias");
		}

		public async Task<Response<Category?>> GetByIdAsync(GetCategoryByIdRequest request)
		{
			var result = await _client.GetFromJsonAsync<Response<Category?>>($"v1/categories/{request.Id}");
			return result ?? new Response<Category?>(null, 400, "Falha ao obter categoria");
		}
